...
# 카테고리 고정 색상(다른 차트에서도 항상 같은 카테고리는 항상 같은 색).
#
# 7색(게이밍 모니터 #8749A4)까지는 validate_palette.js 전 항목을 통과했다
# — 기존 6색 전부와 정상시각 ΔE ≥ 16.6, 색각이상 ΔE ≥ 11.0, 대비 3.21:1.
#
# 8·9색(키보드/마우스)에서 색상환이 한계에 닿았다. OKLCh 격자를 전수 탐색한 결과
# 기존 7색 전부와 정상시각 ΔE ≥ 15를 유지하는 색은 **한 개도 없다**. 아래 2색이
# 측정상 최선이며, 가장 가까운 쌍은 다음과 같다:
#   #454FDB ↔ #3987E5(게이밍 노트북) ΔE 13.6, ↔ #8749A4(게이밍 모니터) ΔE 13.8
#   #A34145 ↔ #D95926(DDR5 RAM) ΔE 13.1, ↔ #D55181(CPU) ΔE 13.1
# 즉 9종부터 색은 보조 단서이고 카테고리 이름이 1차 단서다. 이 저장소에서는 색이
# 쓰이는 모든 자리(CategoryBadge, 막대그래프 X축, 3D 부품 라벨, 부품 패널 제목)에
# 카테고리 이름이 항상 함께 나오므로 색만으로 구분해야 하는 상황은 생기지 않는다.
# 카테고리를 더 늘린다면 색을 짜내지 말고 묶음(예: '입력장치')을 도입해야 한다.
CATEGORY_COLORS = {
    "게이밍 노트북": "#3987E5",
    "DDR5 RAM": "#D95926",
    "NVMe SSD": "#199E70",
